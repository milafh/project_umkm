    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <a href="<?= base_url('admin/produk/form') ?>" class="btn btn-primary float-right"><i class="fa fa-plus"></i></a>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <table id="example1" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        <?php foreach ($data as $key => $value) : ?>
                            <tr>
                                <td><?= ++$key ?></td>
                                <td><?= $value->jenis_produk_nama ?></td>
                                <td><?= $value->kode ?></td>
                                <td>
                                    <img src="<?= base_url('public/produk/'.$value->foto) ?>" class="border" width="32"><br>
                                    <?= $value->nama ?>
                                </td>
                                <td><?= $value->stok ?></td>
                                <td><?= "Rp " . number_format($value->harga_beli,0,',','.') ?></td>
                                <td><?= "Rp " . number_format($value->harga_jual,0,',','.') ?></td>
                                <td><?= substr($value->deskripsi, 0, 15) ?>...</td>
                                <td class="text-center">
                                    <a href="<?= base_url('admin/produk/form/'.$value->id) ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
                                    <a href="<?= base_url('admin/produk/delete/'.$value->id) ?>" onclick="return confirm('Yakin ingin menghapus data?')" class="btn btn-danger"><i class="fa fa-times"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
